...
// An incoming message from the kernel, including leading InHeader
...
// Consume will consume the bytes directly after the InHeader
// struct.
func (m *InMessage) Init(r io.Reader) (err error) {
	...
	const headerSize = unsafe.Sizeof(InHeader{})
...
func (m *InMessage) Header() (h *InHeader) {
	h = (*InHeader)(unsafe.Pointer(&m.storage[0]))
	return
}
